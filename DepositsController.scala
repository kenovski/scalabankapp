 ...
 */
@Singleton
class DepositsController @Inject()(val controllerComponents: ControllerComponents) extends BaseController {
   ...
   * a path of `/`.
   */
  def deposits() = Action { implicit request: Request[AnyContent] =>
   ...
   val tino:Option[Map[String, Seq[String]]]  = request.body.asFormUrlEncoded
    ...
    new BankDeposits(date,cname,acctno,amt).create()
    new Audits(date,cname,acctno,amt,0.0,difx).create()
    ...
    Ok(views.html.deposits(tino,cx,difx))
  }
